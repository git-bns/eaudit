...
RUN mkdir -p /usr/local/tomcat/conf/Catalina/localhost && \
	mkdir -p /usr/local/tomcat/webapps/eaudit && \
...
COPY --from=MAVEN_TOOL_CHAIN /tmp/target/eaudit-1.0.war $TOMCAT_SERVER/usr/local/tomcat/webapps/eaudit.war
COPY --from=MAVEN_TOOL_CHAIN /tmp/target/eaudit-1.0 $TOMCAT_SERVER/usr/local/tomcat/webapps/eaudit/
